...
const zodSchema = z.object({
  themes: z
    .array(
      z.object({
        ...
        headline: z
          .string()
          .describe('Short, one or two sentence headline of theme or idea.'),
      }),
    )
  ...
  score: z
    .number()
    .describe(
      'A score from 0 to 100 for how closely the article aligns with the team context. A low score would mean that the article has some relevance to the team context, but not much. A high score would mean that the article is very relevant to the team context.',
    ),
})
...
export async function processArticles(
  ...
  const analyses = await Promise.all(
    articles.map(async (article) => {
      ...
      await kv.expire(cacheKey, 60 * 60 * 24) // 1 day
